...
public class RetrofitHelper {
	 ...
	 * Retrofit이 이 클래스의 객체를 통해 통신하는 과정을 의미하는 문자열을 log()메서드로 전달한다.
	 */
	...
	/**
	 * 통신 절차를 가로채서 보조적인 처리를 수행하는 클래스 정의 (통신 수행 직전 호출되는 메서드)
	...
	public class MyInterceptor implements Interceptor {
		...
		public Response intercept(Chain chain) throws IOException {
			// 이 위치에서 추가적인 Header 데이터를 통신에 포함시킬 수 있다.
			// UserAgent 값 삽입하기 : 크롬 브라우저의 버전 정보를 삽입하여 웹 서버로부터의 차단 회피하기
			
			Request originalRequest = chain.request();				// 요청 정보 객체 추출
			Request.Builder builder = originalRequest.newBuilder();	// 요청 정보 객체를 복사해 새로운 객체 생성
